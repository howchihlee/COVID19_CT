...
if __name__ == '__main__':
    ...
    pd.DataFrame(val_info).to_csv(os.path.join(_folder_to_save, 'val_info.csv'), index = False)
    pd.DataFrame(test_info).to_csv(os.path.join(_folder_to_save, 'test_info.csv'), index = False)
    ...
    model.load_state_dict(torch.load(os.path.join(_folder_to_save, 'model_init_last.pth') ))
        ...
        for item_train in train_generator:
            ...
            if total_step % 100 == 0:
                ...
                if (epoch > total_epoch // 2) and best_score_ema > val_loss_ema:
                    torch.save(model_ema.state_dict(), os.path.join(_folder_to_save, 'model_best_ema.pth') )
                    best_score_ema = val_loss_ema

        scheduler.step()
        torch.save(model.state_dict(), os.path.join(_folder_to_save, 'model_last.pth'))
        torch.save(model.state_dict(), os.path.join(_folder_to_save, 'model_ema_last.pth'))
