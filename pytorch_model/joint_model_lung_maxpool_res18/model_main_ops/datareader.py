...
constant_ops = Alb.PadIfNeeded(512, 512, border_mode=BORDER_CONSTANT)

train_aug_ops = Alb.Compose([
        Alb.HorizontalFlip(),
        Alb.GridDistortion(border_mode=BORDER_CONSTANT),
        ...
        Alb.ShiftScaleRotate(shift_limit=0.1, scale_limit=0.1, rotate_limit=30, 
                             p=1.0, border_mode=BORDER_CONSTANT),
        Alb.CoarseDropout(),
        Alb.PadIfNeeded(512, 512, border_mode=BORDER_CONSTANT)
         ], p=1)
...
class DataReader(tdata.Dataset):
    def __init__(self, data, pid2features, transform = None):
        super(DataReader, self).__init__()
        self.data = data ## list of (pid, filename, label)
        self.transform = transform
        self.num_sample = len(self.data)
        self.pid2features = pid2features
    ...
    def __getitem__(self, index):  
        ...
        img[:, :, 2] = skimage.morphology.dilation(img[:, :, 2], selem=np.ones((7, 7)))
        img = img[:, :, 0] * img[:, :, 2]
        img = proc_img(img[:, :, np.newaxis], transform = self.transform)
        feature = self.pid2features[pid]
        
        return img, feature.astype('float32'), np.float32([label])
